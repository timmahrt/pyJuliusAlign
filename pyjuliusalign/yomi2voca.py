...
#
# The code was adapted from this file in particular: https://github.com/julius-speech/segmentation-kit/blob/master/segment_julius.pl
...
def convert(txt):
    ...
    def replace(txt, replDict):
        ...
        for src, tgt in replDict.items():
            txt = txt.replace(src, " %s " % tgt)
        return txt

    txt = replace(txt, threeToOne)
    txt = replace(txt, twoToOne)
    txt = replace(txt, oneToOne)

    txt = re.sub(r'[_\W]+', ' ', txt).strip()
