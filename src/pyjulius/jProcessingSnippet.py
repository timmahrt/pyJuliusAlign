...
#             k, r, c = [unicode(item, 'utf-8') for item in [k, r, c]]
        if k == 'X':
            continue
        romaji = ''.join([item.replace('X', '') for item in 
         [r, c]])
        chartDict[k] = romaji
...
class CabochaNotRunError(Exception):
    ...
    def __str__(self):
        return u"Cabocha not installed or not in path.  Attempted to run command:\n%s" % self.cabochaCmd
...
class CabochaOutputError(Exception):
    ...
    def __str__(self):
        return "Unexpected error in cabocha output (possibly a problem with cabocha).  See error:\n%s" % self.outputStr
...
#     validHiraganaList = [u"ゃ", u"ゅ", u"ょ", u"っ"] + kanaToRomajiDict.keys()
    for chunk in sentence:
        ...
        for tok in chunk.findall('tok'):
            ...
            featureStr = tok.get("feature") # Contains syntactic and morphological information
            featureList = featureStr.split(',')
            if featureList[0] == u"記号":
                continue

            # Don't process empty words (can happen a lot, depending on the annotation scheme)
            word = tok.text.strip()            
            if word == "":
                continue
            ...
            if kana != '*':
                ...
                # Map all hiragana to katakana (in what situations would a hiragana word not appear in our dictionary?
                word = convertKanaToKata(word)

                # If the text is all katakana, keep it, otherwise, assume an error
...
def getChunkedKana(string, cabochaEncoding, cabochaPath):
    ...
    vowelModifierDict = {u"ァ":u"a", u"ィ":u"i", u"ゥ":u'u', u"ェ":u"e", u"ォ":'o'}
    ...
    for word in kanaList:
        ...
        for kana in kanaInputList:
            ...
            # Last vowel is a long vowel
                ...
                # e.g. 'ホーー' becomes "ho::" or 'ふうんー' becomes "hu:n:" -- both bad
                if romanjiList[-1][-1] != ":" and romanjiList[-1][-1] != "N":
                    romanjiList[-1] += ":"
                pass # Should get recognized as a single vowel by the forced aligner
            
            # Normal case
            else:
                if kana == u"ン" or kana in vowelList: # Single-phone characters
                    ...
                    if kana in yModifierDict.keys(): # e.g. 'ィ' in 'ティム'
                        ...
                        try:
                            syllable = kataToRomajiDict[kana] # Normal case for two-phone characters
                        except KeyError:
                            raise NonKatakanaError(kana, string)
                        
                    romanjiList.append(syllable[:-1]) # The consonant (e.g. 's' or 'sh')
                    romanjiList.append(syllable[-1]) # The vowel
...
def chunkKatakana(kanaStr):
    ...
    while i < len(kanaStr):
        kana = kanaList[i]
        # Some palatalized consonants are in our chart (notably 'jy' and 'shy' are missing)
        if kana in [u"ャ", u"ュ", u"ョ"] and returnList[-1]+kana in kataToRomajiDict.keys():
            returnList[-1] += kana
...
#         print getChunkedKana(textStr)
        print cabocha(textStr, cabochaEncoding, cabochaPath).encode('utf-8')
        print "\n"
